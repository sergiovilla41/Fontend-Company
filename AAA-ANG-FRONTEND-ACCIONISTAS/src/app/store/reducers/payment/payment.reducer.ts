...
import { UpdateStatePayment, deletePaymentError, deletePaymentSuccess, updatePaymentError, updatePaymentSuccess } from '../../actions/payment.action';
...
export const paymentReducer = createReducer(
  paymentInitialState,
    on(updatePaymentSuccess, (state, { msg, status }) => {
        return { ...state, msg, status }
    }),
    on(updatePaymentError, (state, { msg, status }) => {
        return { ...state, msg, status }
    }),
    on(UpdateStatePayment, (state) => {
        return { status: null }
    }),
    on(deletePaymentSuccess, (state, { msg, status }) => {
        return { ...state, msg, status }
    }),
    on(deletePaymentError, (state) => {
        return { status: null }
    }),
